...
// Expanded FAANG-level catalog: 34 patterns x 20 LeetCode problems = 680 linked problems.
  ...
  const problemSlug = title => title.toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  for (const [pattern, rows] of Object.entries(EXPANDED_PROBLEM_ROWS)) {
    PROBLEMS_DB[pattern] = rows.split(';').map(row => {
      const [title, difficulty, slug] = row.split('|');
      return { title, difficulty, url: `https://leetcode.com/problems/${slug || problemSlug(title)}/` };
    });
  }

  // Phase definitions
  const PHASES_DATA = [
    { id:1, label:"Phase 1 - Foundation", color:"#3B8BD4", patterns:["Two Pointers","Sliding Window","Prefix Sum","Cyclic Sort"] },
    ...
    { id:5, label:"Phase 5 - Advanced", color:"#7F77DD", patterns:["Trie","Backtracking","Two Heaps","Bit Manipulation","Greedy Patterns","Segment Tree / BIT"] }
  ];
  ...
  for(let phase of PHASES_DATA){
    for(let pname of phase.patterns){
      let probs = PROBLEMS_DB[pname] || [{ title:`Practice ${pname}`, url:"#", difficulty:"medium" }];
      patternsLibrary.push({
        ...
        color: phase.color,
        problems: probs.map((p,idx) => ({ id:`${pname}_${idx}`, title:p.title, url:p.url, difficulty:p.difficulty, completed:false, tags:[], notes:"", videoUrl:"" }))
      });
    }
  }
  ...
  function applyProgressRecords(records){
    let byId = new Map((records || []).map(item => [item.problemId, item]));
    for(let pattern of patternsLibrary){
      for(let problem of pattern.problems){
        let saved = byId.get(problemKey(pattern, problem));
        if(!saved) continue;
        problem.completed = saved.completed === true || saved.completed === "TRUE" || saved.completed === "true";
  ...
  async function loadState(){
    let stored = localStorage.getItem("dsa_faang_state");
    if(stored){
    ...
    for(let pat of patternsLibrary){
      for(let prob of pat.problems){
        let key = `${pat.id}_${prob.id}`;
        if(appState.problemStatus[key]) {
          prob.completed = appState.problemStatus[key].completed;
          prob.notes = appState.problemStatus[key].notes;
          prob.videoUrl = appState.problemStatus[key].videoUrl;
          if(appState.extraTags[key]) prob.tags = [...new Set([...prob.tags, ...appState.extraTags[key]])];
        } else appState.problemStatus[key] = { completed:false, notes:"", videoUrl:"" };
      }
    }
  ...
  function persist(){
    for(let pat of patternsLibrary){
      for(let prob of pat.problems){
        let key = `${pat.id}_${prob.id}`;
        appState.problemStatus[key] = { completed:prob.completed, notes:prob.notes, videoUrl:prob.videoUrl };
        let customTags = prob.tags.filter(t => !['easy','medium','hard'].includes(t));
        if(customTags.length) appState.extraTags[key] = customTags;
      }
    }
    localStorage.setItem("dsa_faang_state", JSON.stringify({ problemStatus:appState.problemStatus, extraTags:appState.extraTags }));
    queueSheetsSave();
    updateChart();
  }
  ...
  function renderAllPatterns(){
    ...
    let filtered = patternsLibrary.filter(p => p.name.toLowerCase().includes(searchTerm) || p.problems.some(pr => pr.title.toLowerCase().includes(searchTerm)));
    let map = new Map();
    ...
    for(let ph of map.values()){
      ...
      html += `<div class="phase-block"><div class="phase-header" onclick="togglePhaseGrid(this)"><div class="phase-dot" style="background:${ph.color}"></div><div class="phase-title">${ph.label}</div><div class="phase-progress">${solvedProb}/${totalProb} solved</div><i class="ti ti-chevron-down"></i></div><div class="pattern-grid" style="display:grid;">`;
      for(let pat of ph.patterns){
        ...
        html += `<div class="pattern-card ${appState.selectedPatternId===pat.id?'selected':''}" data-pattern-id="${pat.id}"><div class="pattern-title-row"><strong>${pat.name}</strong><span class="text-small pattern-count">${solved}/${total}</span></div><label class="pattern-check" onclick="event.stopPropagation();"><input type="checkbox" class="pattern-master-check" data-id="${pat.id}" ${checkedAttr} onchange="togglePatternMaster(${pat.id})"><span class="text-small">Mark all solved</span></label><div class="pattern-progress"><div class="pattern-progress-meta"><span class="text-small">Progress</span><span class="text-small">${percent}%</span></div><div class="pattern-progress-track"><div class="pattern-progress-fill" style="width:${percent}%"></div></div></div></div>`;
      }
      html += `</div></div>`;
    }
    container.innerHTML = html;
    document.querySelectorAll('.pattern-card').forEach(card => {
      card.addEventListener('click', (e) => { if(e.target.tagName!=='INPUT'){ appState.selectedPatternId = parseInt(card.dataset.patternId); renderAllPatterns(); renderProblemList(patternsLibrary.find(p=>p.id===appState.selectedPatternId)); } });
    });
  }
  ...
  function renderProblemList(pattern){
    if(!pattern){ document.getElementById('problemListContainer').innerHTML='<div class="empty-state">Select a pattern to start solving</div>'; document.getElementById('selectedPatternName').innerText="No pattern selected"; document.getElementById('patternStats').innerText="0/0"; return; }
    document.getElementById('selectedPatternName').innerHTML=`<i class="ti ti-puzzle"></i> ${pattern.name}`;
    let solved = pattern.problems.filter(p=>p.completed).length;
    document.getElementById('patternStats').innerHTML=`${solved}/${pattern.problems.length} solved`;
  ...
  window.promptTag = (pid, probId) => { let pat=patternsLibrary.find(p=>p.id===pid); let prob=pat.problems.find(p=>p.id===probId); let newTag=prompt("Add tag (e.g. 'two-pointer', 'sliding-window'):"); if(newTag) { prob.tags.push(newTag); persist(); renderProblemList(pat); renderAllPatterns(); } };
  window.promptNote = (pid, probId) => { let pat=patternsLibrary.find(p=>p.id===pid); let prob=pat.problems.find(p=>p.id===probId); let note=prompt("Add notes / solution insight:", prob.notes); if(note!==null) { prob.notes=note; persist(); renderProblemList(pat); } };
  window.promptVideo = (pid, probId) => { let pat=patternsLibrary.find(p=>p.id===pid); let prob=pat.problems.find(p=>p.id===probId); let url=prompt("Add video / resource URL:", prob.videoUrl); if(url!==null) { prob.videoUrl=url; persist(); renderProblemList(pat); } };
  window.togglePhaseGrid = (header) => { let grid=header.nextElementSibling; let icon=header.querySelector('.ti-chevron-down'); if(grid.style.display==='none'){ grid.style.display='grid'; icon.style.transform='rotate(0deg)'; } else { grid.style.display='none'; icon.style.transform='rotate(-90deg)'; } };
  function updateChart(){
    let solved=0, total=0;
    patternsLibrary.forEach(p=>{ p.problems.forEach(prob=>{ total++; if(prob.completed) solved++; }); });
    document.getElementById('totalSolved').innerText=solved; document.getElementById('totalProblems').innerText=total;
    if(chart) chart.destroy();
    let ctx = document.getElementById('progressChart').getContext('2d');
    chart = new Chart(ctx,{ type:'doughnut', data:{ labels:['Solved','Remaining'], datasets:[{ data:[solved, total-solved], backgroundColor:['#3b82f6', isNightMode() ? '#29354a' : '#e2e8f0'], borderWidth:0 }] }, options:{ responsive:true, maintainAspectRatio:true, cutout:'60%', plugins:{ legend:{ position:'bottom', labels:{ color:isNightMode() ? '#cbd5e1' : '#334155', boxWidth:10, font:{size:9} } } } } });
  }
  function renderAll(){ renderAllPatterns(); if(appState.selectedPatternId!==null){ let selected=patternsLibrary.find(p=>p.id===appState.selectedPatternId); if(selected) renderProblemList(selected); else document.getElementById('problemListContainer').innerHTML='<div class="empty-state">Select a pattern</div>'; } else { document.getElementById('problemListContainer').innerHTML='<div class="empty-state">Click any pattern to start solving</div>'; } updateChart(); }
